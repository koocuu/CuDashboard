...
- **2026-08-13**:顶栏改为「今日 / 项目 / 画像」；选题退出导航与首页主场，仅备查；作品墙为独立对象，MCP `get_projects` 可读；台账是季节性捕捉面，闲时不占主身份；作品墙不分在做/上线/暂停，用大字目录切换单件详情
...
**C. 作品墙**
coding 作品，与工作事项分开。不分在做/上线/暂停。人和 AI 读同一份（页面 + MCP `get_projects`）。不自动写入画像层。
...
projects
  id, name, slug, status(列保留兼容,界面与 MCP 不展示/不筛选),
  area(personal/work/writing),
...
顶栏画像角标；投资待审时「投资」Tab 显示角标。今日页拆成画像/投资两条待确认。MCP 返回审批直达链接。pending 超过 7 天标过期且不可直接批准；同月新月度提案会自动拒绝旧 pending。详情页展示当前版 vs 提案版逐行 diff → 可编辑后批准 → 旧版入 profile_versions(可回滚),version+1。不发微信推送。
...
顶栏:今日 / 项目 / 投资 / 画像。选题不进导航。投资待审时在「投资」上显示角标。
...
4. **备查链**:顶栏右侧淡链「选题」，不与主导航并列
作品列表只在「项目」页，仓位只在「投资」页，今日不再重复一份。
